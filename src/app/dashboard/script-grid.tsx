...
export default function ScriptGrid({ scripts }: { scripts: Script[] }) {
  ...
  const handleSendSelected = async () => {
    ...
    setIsSending(false)
    if (res.success) {
      setMessage(`EMAILED ${selectedIds.length} SCRIPTS`)
      setSelectedIds([])
  ...
  const handleDeleteSelected = async () => {
    if (selectedIds.length === 0) return
    if (!confirm(`Delete ${selectedIds.length} scripts? This cannot be undone.`)) return
    ...
    if (res.success) {
      setMessage(`Deleted ${selectedIds.length} scripts`)
      setSelectedIds([])
      setTimeout(() => setMessage(''), 5000)
    } else {
      setMessage(`ERROR: ${res.error}`)
        ...
        <div className="flex items-center gap-12">
            ...
            <div className="flex flex-col">
                <span className="text-[12px] font-bold text-white uppercase tracking-[0.3em]">
                    <span className="text-white text-xl mr-4">{selectedIds.length.toString().padStart(2, '0')}</span> SELECTED
                </span>
            </div>
        </div>
          ...
          >
            ...
            <div className="mt-12 mb-10 flex flex-col gap-6">
                <div className="flex items-center gap-4">
                    <span className="text-[10px] font-black uppercase tracking-[0.6em] text-white/60">{script.platform}</span>
                </div>
            ...
            >
              ...
              <div className="px-10 py-10 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
                <div className="flex items-center gap-10">
                    ...
                    <div className="space-y-1">
                        <div className="flex items-center gap-4">
                            <span className="text-[9px] font-black text-white/70 uppercase tracking-[0.6em]">{selectedScript.platform}</span>
                            <div className="h-1 w-1 bg-white animate-pulse" />
                            <span className="text-[9px] font-black text-white/70 uppercase tracking-[0.6em]">{selectedScript.tone} Tone</span>
                        </div>
              ...
              <div className="flex-1 overflow-y-auto p-12 lg:p-16 scrollbar-thin scrollbar-thumb-white/5 custom-scrollbar bg-black">
                  <div className="relative group">
                    <div className="absolute -top-3 left-10 bg-black px-4 text-[8px] font-black text-white/60 uppercase tracking-[0.8em] z-10 transition-colors group-focus-within:text-white">Content</div>
                  ...
                  <button 
                    onClick={async () => {
                      const res = await sendSelectedScripts([selectedScript.id])
                       if (res.success) setMessage('EMAILED SCRIPT')
                    }}
